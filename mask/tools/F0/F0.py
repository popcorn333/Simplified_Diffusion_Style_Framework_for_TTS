...
def calculate(
    ...
    for i, gen_path in enumerate(file_list):
        ...
        #save persample result
        gen_txt_path = os.path.splitext(gen_path)[0] + "_f0" + ".txt"
        with open(gen_txt_path, "w") as f:
            f.write(f"{log_f0_rmse:.4f}\n")
...
def main():
    ...
    with open(f"{args.outdir}/F0.txt", "a") as f:
        f.write(f"{args.gen_wavdir_or_wavscp}   {mean_log_f0_rmse:.4f} \\pm {std_log_f0_rmse:.4f}\n")
