...
class Forward_Diffusion(BaseModule):
    def __init__(self, cfg):
        ...
        self.alpha = cfg.model.Masking.alpha
    ...
    def forward(self, X0, mask, mu, n):
        device = X0.device
        X_shape = X0[0].shape
        Xn = torch.ones(X0.shape, requires_grad=True).to(device) 
        terms = torch.ones(X0.shape, requires_grad=True).to(device)  # Creating the terms tensor on GPU
        ...
        Xm = torch.ones(X0.shape, requires_grad=True).to(device)
        X_itm.retain_grad()
        Xm.retain_grad()
        Xn.retain_grad()
        terms.retain_grad()
...
class ReversedForward_Diffusion(nn.Module):
    ...
    def forward(self, x, mask, mu, n):
        ...
        alpha = alpha = torch.tensor([self.alpha]).cuda() 
...
class Diffusion(BaseModule):
    def __init__(self, cfg):
        ...
        self.forward_pass = Forward_Diffusion(cfg)
